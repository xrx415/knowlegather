...
import MDXViewer from '../../components/markdown/MDXViewer';
...
interface ResourceViewerMDXProps {
  resource: Resource | null;
  onAddResource?: () => void;
}

/**
 * ResourceViewerMDX - Wersja viewera używająca MDX
 * 
 * Równoległa implementacja do ResourceViewer.tsx
 * Używa MDXViewer zamiast react-markdown, co pozwala na:
 * - Używanie React komponentów w markdown
 * - Tworzenie interaktywnych "Klocków"
 * - Rozszerzalność przez custom komponenty
 */
const ResourceViewerMDX = ({ resource, onAddResource }: ResourceViewerMDXProps) => {
  ...
  // Custom komponenty które będą dostępne w MDX ("Klocki")
  const customComponents = {
    AlertBox,
    Counter,
    // TODO: Dodać Excalidraw, KalkulatorTreningowy itp.
  };
    ...
    <div className="flex flex-col h-full overflow-hidden">
      ...
      <div className="px-6 py-3 bg-gray-50 border-t border-gray-200 flex justify-between items-center fixed left-0 right-0 bottom-0 z-40" style={{maxWidth: 'calc(100vw - 17rem)', marginLeft: '16rem'}}>
        <div className="text-sm text-gray-500">
          {resource.is_note ? 'Notatka użytkownika' : 'Pobrany zasób'} · <span className="text-primary-600 font-medium">Renderer: MDX</span>
        </div>
...
export default ResourceViewerMDX;
